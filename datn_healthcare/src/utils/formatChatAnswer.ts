...
const renderHtmlTable = (headerCells: string[], bodyRows: string[][]): string => {
  ...
  const equalColumnCount = Math.max(headerCells.length - 1, 1);
  const firstColumnWidth = 76;
  const equalColumnWidth = `calc((100% - ${firstColumnWidth}px) / ${equalColumnCount})`;
  const colGroup = headerCells
    .map((_, index) =>
      index === 0
        ? `<col style="width:${firstColumnWidth}px;" />`
        : `<col style="width:${equalColumnWidth};" />`
    )
    .join("");
  const ths = headerCells
    .map(
      (c) =>
        `<th style="padding:7px 9px;border:1px solid #dbe4ff;background:#eef4ff;text-align:left;font-size:12px;line-height:1.35;vertical-align:top;word-break:break-word;overflow-wrap:anywhere;">${inlineFormat(
          escapeHtml(c)
        )}</th>`
    )
    .join("");
  const trs = normalizedBodyRows
    .map((row) => {
      const tds = row
        .map(
          (c) =>
            `<td style="padding:7px 9px;border:1px solid #e2e8f0;font-size:12px;line-height:1.45;vertical-align:top;word-break:break-word;overflow-wrap:anywhere;">${inlineFormat(
              escapeHtml(c)
            )}</td>`
        )
        .join("");
    ...
    .join("");
  return `<div class="chat-answer-table-wrap"><table style="border-collapse:collapse;width:100%;min-width:100%;table-layout:fixed;margin:10px 0;"><colgroup>${colGroup}</colgroup><thead><tr>${ths}</tr></thead><tbody>${trs}</tbody></table></div>`;
};
