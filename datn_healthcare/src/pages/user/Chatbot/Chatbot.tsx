...
import type { ChatMessage, ChatMeta, ChatSource, RagHealth, StoredChatSession, StoredChatState } from "@/types/ChatType";
...
const Chatbot: React.FC = () => {
  ...
  const streamAssistantReply = (assistantMessage: ChatMessage) =>
    new Promise<void>((resolve) => {
      ...
      const interval = total > 220 ? 10 : 14;
      ...
      if (total === 0) {
        ...
        let index = 0;
        setTypingAssistant({ id: assistantMessage.id, content: "", meta: assistantMessage.meta as ChatMeta });
        scrollToBottom("smooth");

        const tick = () => {
        index = Math.min(total, index + step);
        typingBufferRef.current = chars.slice(0, index).join("");
        setTypingAssistant({
          id: assistantMessage.id,
          content: typingBufferRef.current,
          meta: assistantMessage.meta as ChatMeta,
        });
        scrollToBottom("auto");

        if (index < total) {
          typingTimerRef.current = window.setTimeout(tick, interval);
          return;
        }
  ...
  useEffect(() => {
    scrollToBottom(typingAssistant ? "auto" : "smooth");
  }, [messages, loading, typingAssistant]);
  ...
  */
  ...
  const handleSend = async (rawPrompt?: string) => {
    ...
    try {
      ...
      await streamAssistantReply(assistantMessage);
    } catch (error: any) {
  ...
  const handleCloseFullChat = () => {
    ...
    window.setTimeout(() => {
      navigate(from && from !== location.pathname ? from : "/user");
    }, 180);
  };
  ...
  return (
    <div className={`chat-page${isClosingPage ? " is-leaving" : ""}`}>
      <div className="chat-layout">
        ...
        <section className="chat-main">
          ...
          <div className="chat-scroll" ref={scrollRef} onScroll={handleScroll}>
            ...
            {typingAssistant && (
              <div className="chat-bubble-row chat-bubble-row--assistant chat-bubble-row--typing">
                <div className="chat-avatar">
                  <Stethoscope size={16} />
                </div>
                <div className="chat-bubble-col">
                  <div className="chat-bubble chat-bubble--assistant chat-bubble--typing">
                    <span className="chat-typing-text">{typingAssistant.content}</span>
