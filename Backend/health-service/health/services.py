...
def predict(features: dict[str, float]) -> dict[str, Any]:
    ...
    local_prediction = _local_model_api_prediction(features)
...
def _local_model_api_prediction(features: dict[str, float]) -> dict[str, Any] | None:
    try:
        from model_api.main import predict_all

        data = predict_all(features)
    except Exception:
        return None

    if isinstance(data, dict) and all(key in data for key in ("diabetes", "cardio", "stroke")):
        return data
    return None
