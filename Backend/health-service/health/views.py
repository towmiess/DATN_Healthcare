...
def _model_api_ocr_text(image_base64: str, mime_type: str) -> str:
    ...
    if not model_url:
        return _local_model_api_ocr_text(image_base64, mime_type)
...
def _local_model_api_ocr_text(image_base64: str, mime_type: str) -> str:
    try:
        from model_api.main import OCRRequest, ocr_google_vision

        data = ocr_google_vision(OCRRequest(image_base64=image_base64, mime_type=mime_type))
    except Exception:
        return ""
...
class ReportAiInsightView(APIView):
    def post(self, request):
        serializer = AiInsightRequestSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        user = ensure_shadow_user(request)
        try:
            payload = generate_ai_insight(
                user=user,
            ...
            return Response(
                {"message": "Không thể đọc dữ liệu sức khỏe để tạo AI Insights."},
